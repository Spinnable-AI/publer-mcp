...
async def publer_check_account_status(ctx: RequestContext) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        user_info = await client.get("users/me", ctx.headers, include_workspace_header=False)
        
        # Get workspace information (tests workspace ID authentication)
        workspaces = await client.get("workspaces", ctx.headers, include_workspace_header=False)
        
        # Find current workspace from header
        workspace_id = ctx.headers.get('x-workspace-id')
...
async def publer_list_connected_platforms(ctx: RequestContext) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        accounts_response = await client.get("accounts", ctx.headers)
        ...
        elif "Missing x-workspace-id" in str(e):
            return {
                "status": "workspace_required",
                "error": "Workspace ID is required to list connected platforms.",
                "platforms": []
            }
        else:
            return {
